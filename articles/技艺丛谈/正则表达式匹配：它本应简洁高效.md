...
正则表达式是一种描述字符串集的表示法。当一个特定的字符串在正则表达式描述的集合中，我们说该正则表达式**匹配**该字符串。

最简单的正则表达式为单一常量字符。除特殊的元字符`*+?()|`外，其它字符都可以匹配自身。如果要匹配一个元字符，可以使用反斜杠来转义：`\+`  匹配常量字符`+`。 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTA2ODEwNjQ0NywzNTQ2NTAxMTVdfQ==
-->