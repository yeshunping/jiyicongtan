...
- 我们使用的一些数据集非常庞大，远远超出了单机内存。随着最近固态磁盘（SSD）可用性的增强，使用SSD而不是内存将数据集进一步扩展，将是很有吸引力的。为此，我们添加了**显式映射一个或多个文件的能力**，而不是使用匿名mmap() 函数。到目前为止，我们的实验表明，对于适合放在内存的数据集的应用来说，这是一种很有前景的方法，但我们仍在分析是否可以充分利用这种方法来证明SSD的成本。
...
略。
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE2OTg3MzY5ODAsMTk3NjM2NDY2NSwtMj
