...
-   活跃的内存和内存使用必须有**稳定**的关系。（译注：使用tcmalloc的术语就是，in use by application 不变的情况下，total memory 需要保持基本不变，否则内存分配器本身的overhead就越来越大了）。换言之，内存分配器引入的内存碎片的实际代价是非常重要的。想象一下，如果内存碎片使得内存使用每天增加1G，一个设计的时候剩余不多内存的话，将会来几天内就挂了。（译注：比如预计是4G，机器才8G，4天就因为内存不够挂了）
...
略。
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTMwNjQ5Mzk5LDQ0NzkxOTc3NCwxNDAwMj
