...
-   **严格控制分配器元数据本身的额外代价**。除了内存碎片，jemalloc 控制元数据(metadata)的额外代价在总内存使用的2%以内，用于存储不同大小的类别。(译注：主要是内存分配器本身使用的内存，比如用于存储缓存对象的链接等数据结构)
...
略。
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTEyNDY4ODYzNyw1ODcyNTM1NTEsNDQ3OT
