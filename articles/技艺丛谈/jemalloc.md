...
线程缓存的主要目的在于降低同步事件的数量。因此，每个大小类别所缓存的对象最大数量，应该限制在一个允许在实践中减少同步事件10-100倍的水平。更高的缓存限制也许能为某些应用带来进一步的分配加速，但是一般会带来不可接受的内存碎片代价。为了进一步降低
...
略。
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTQ2MDIyNTU5NCwtNTY5MDY4MTIwLDExND
Q1NTIyMDFdfQ==
-->