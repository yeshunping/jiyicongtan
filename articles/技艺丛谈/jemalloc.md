...
每个arena块包含metadata信息（主要是页面映射），后面是一到多个连续页（译注：page runs，找不到合适的翻译词，这里翻译为连续页)。小对象被组织到一起，在每个连续页的起始地址存放额外的metadata。而大对象是彼此独立的，他们的 metadata 完全存放在arena块的头部。每个arena块使用红黑树来记录未使用完的小对象连续页（每个大小的列表使用一棵红黑树），
...
略。
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTgyMDk1NDYzOSwtNTY5MDY4MTIwLDExND
Q1NTIyMDFdfQ==
-->