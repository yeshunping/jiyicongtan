...
每个线程维护一个小对象的缓存，以及最大到一定大小的大对象（默认是32K）。因此大部分分配请求，首先检测是否有可获取的缓存对象，然后再访问 arena。通过线程缓存的分配，是不需要任何锁的，而通过arena 来分配是需要锁住一个 arena 箱子（每个小的大小类别），和（或）arena 整体。
...
略。
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEzMzg1MDM2NSwtNTY5MDY4MTIwLDExND
Q1NTIyMDFdfQ==
-->