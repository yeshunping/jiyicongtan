...
Let's use superscripts to denote string repetition, so that  `a?3a3`  is shorthand for  `a?a?a?aaa`. The two graphs plot the time required by each approach to match the regular expression  `a?`<sup>n</sup>`a`<sup>n</sup>  against the string  `a`<sup>n</sup>.
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbNjI2ODQ4MjAwLDE3MjE4NjI0NzBdfQ==
-->